...
if ( ! function_exists( 'ogape_send_waitlist_confirmation_email' ) ) {
    function ogape_send_waitlist_confirmation_email( $entry ) {
        ...
        $whatsapp_url  = ogape_get_whatsapp_url();
        $year          = gmdate( 'Y' );

        $has_referral = ! empty( $entry['referral_code'] );
        $referral_url = $has_referral ? home_url( '/ref/' . rawurlencode( sanitize_text_field( $entry['referral_code'] ) ) ) : '';

        $referral_plain = $has_referral
            ? "\n\n02 Avanzá en la lista\nInvitá a alguien usando tu enlace y subís posiciones automáticamente.\n" . $referral_url
            : '';

        $referral_html = $has_referral
            ? '<p style="margin:18px 0 8px;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#5d5d5d;">SUMÁ GENTE, AVANZÁS VOS</p>'
              . '<p style="margin:0 0 8px;font-size:16px;font-weight:600;">Compartí tu lugar</p>'
              . '<p style="margin:0 0 14px;font-size:14px;line-height:1.6;">Cada persona que se anote con tu enlace te acerca más al acceso.</p>'
              . '<p style="margin:0 0 16px;padding:10px 12px;background:#f5f3ec;border:1px dashed #c8c2b2;border-radius:10px;font-size:14px;word-break:break-all;">' . esc_url( $referral_url ) . '</p>'
              . '<p style="margin:0 0 18px;"><a href="' . esc_url( $referral_url ) . '" style="display:inline-block;background:#1f1f1f;color:#ffffff;text-decoration:none;padding:12px 16px;border-radius:10px;font-size:14px;font-weight:600;">Copiar mi enlace</a></p>'
            : '';

        $plain_message = sprintf(
            "OGAPE\nTU CHEF EN CASA\nLISTA CONFIRMADA\n\nHola %1\$s,\n\nYa sos parte de lo que viene.\n\nGracias por anotarte. Estamos preparando algo cuidado — y vas a ser de los primeros en probarlo.\n\nLo que sigue\n01 Te avisamos cuando esté listo\nEl acceso se abre por etapas. Te escribimos a %2\$s cuando llegue tu turno.%3\$s\n\n03 Acceso anticipado\nLos primeros en entrar tienen condiciones especiales del lanzamiento piloto.\n\nINGREDIENTES FRESCOS\nCocina de calidad, en tu casa.\nIdentidad paraguaya. Presentación contemporánea. Conveniente por diseño.\n\nSeguinos mientras esperás\nInstagram · Facebook · WhatsApp\n\nCancelar suscripción · Política de privacidad: %4\$s\n© %5\$s Ogape Paraguay. Todos los derechos reservados.",
            $name,
            $email,
            $referral_plain,
            $privacy_url,
